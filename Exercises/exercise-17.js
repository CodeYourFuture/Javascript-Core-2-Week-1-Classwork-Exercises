  ...
  */
  ...
  let restaurantFinderApplication = {
    ...
    findAvailableRestaurants: function (numberOfPeople) {
            return restaurants.filter((el) => el.totalSeats - el.numberOfCustomers > numberOfPeople)
            .map((el) => el.name);
      // Complete here
    },
    findRestaurantServingDish: function (dishName) {
      return restaurants.filter((el) => el.menu.includes(dishName))
      .map((el) => el.name); // Complete here
    },
    countNumberOfRestaurantsInArea: function (area) {
      return restaurants.filter((el) => el.address.area.includes(area))
      .map((el) => el.name); // Complete here
    },
  };
